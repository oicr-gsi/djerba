...
class Pull():
    ...
    #def __init__(self, db='djerba_test', base='http://admin:djerba123@10.30.133.78:5984/', level=logging.INFO, log_path=None, filename=__name__):
        self.db = db
        self.base = base
    ...
    def TerminalPrint(self,search, number, files, view, equal=None):
        ...
        # print(f'Files Found = {len(files)} / View: <{view}>')
        time = datetime.now()
        ...
        header.insert(0, '#')
        #print(tabulate(df, headers=header, showindex=True, tablefmt='fancy_grid'))
        print(tabulate(df, headers=header, showindex=True))
        if len(search) > 1 and view != search[-1]: print()
        return
