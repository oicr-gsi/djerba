...
sample_tcga <- tolower(ifelse(clinicalData$CLOSEST_TCGA == "COAD" | clinicalData$CLOSEST_TCGA ==
  ...
  sample_pct_type <- ifelse(sample_tcga %in% external_tmb_data_type$CANCER.TYPE, ecdf(external_tmb_data_type$tmb)(sampleTMB),
                            ifelse(sample_tcga %in% tcga_tmb_data$CANCER.TYPE, ecdf(tcga_tmb_data_type$tmb)(sampleTMB),
                                   NA))
...
# join maf & CNV
if (dim(cbioMAF)[1] != 0) {
  out1 <- left_join(cbioMAF, CNA) %>%
    select(Hugo_Symbol, Chromosome, HGVSp_Short, Variant_Classification, tumour_vaf,
      t_depth, t_alt_count, Copy_State, OncoKB) %>%
    mutate(tumour_vaf = round(tumour_vaf, 2), tumour_vaf = (tumour_vaf * 100),
      Copy_State = base::gsub("0", "Neutral", Copy_State), Copy_State = base::gsub("^1",
        "Gain", Copy_State), Copy_State = base::gsub("^2", "Amplification",
        Copy_State), Copy_State = base::gsub("^-1", "Shallow Deletion", Copy_State),
      Copy_State = base::gsub("^-2", "Deep Deletion", Copy_State)) %>%
    mutate(OncoKB = base::gsub("LEVEL_", "Level ", OncoKB)) %>%
    mutate(Variant_Classification = base::gsub("_", " ", Variant_Classification)) %>%
    mutate(Depth = paste(t_alt_count, t_depth, sep = "/")) %>%
    rename(Gene = Hugo_Symbol, Type = Variant_Classification, HGVS = HGVSp_Short,
      `Copy State` = Copy_State, `VAF (%)` = tumour_vaf) %>%
    select(-c(t_depth, t_alt_count)) %>%
    relocate(Depth, .after = `VAF (%)`) %>%
...
if (dim(cbioCNV)[1] != 0) {
  CNV <- cbioCNV %>%
    select(Hugo_Symbol, Chromosome, Alteration, OncoKB) %>%
    mutate(OncoKB = base::gsub("LEVEL_", "Level ", OncoKB)) %>%
...
defTable <- rbindlist(list(out1, CNV), fill = TRUE) %>%
  select(Gene, Chromosome) %>%
  left_join(oncoKBsummaries) %>%
  unique() %>%
  rename(Summary = summary) %>%
  mutate(Summary = base::gsub("^NA", "No information currently available in OncoKB.",
    Summary))

defTable <- defTable[order(Gene), ]

Tx <- rbindlist(list(cbioMAFTx, cbioCNVTx), fill = TRUE) %>%
  select(-c(Highest_level, Chromosome)) %>%
  rename(`Gene(s)` = Hugo_Symbol) %>%
  unite(`Treatment(s)`, LEVEL_1:LEVEL_R3, na.rm = TRUE) %>%
  mutate(Alteration = paste0("[", Alteration, "](", "https://www.oncokb.org/gene/",
    `Gene(s)`, "/", Alteration, "/", clinicalData$CANCER_TYPE_DETAILED, ")")) %>%
  mutate(`Gene(s)` = paste0("[", `Gene(s)`, "](", "https://www.oncokb.org/gene/",
    `Gene(s)`, ")")) %>%
  mutate(Alteration = base::gsub("/Deep Deletion", "/Truncating%20Mutations", Alteration)) %>%
  mutate(`Treatment(s)` = base::gsub(",", ", ", `Treatment(s)`))

Tx <- Tx[order(ordered(Tx$OncoKB, levels)), ]

FDA <- subset(Tx, OncoKB %in% c("LEVEL_1", "LEVEL_2", "LEVEL_R1")) %>%
  mutate(OncoKB = base::gsub("LEVEL_", "Level ", OncoKB))
...
<table width="100%">
  <tr>
    <td width="33%"><strong>OncoTree code:</strong> <b>*** PLACEHOLDER ***</b></td> <!-- was clinicalData$CANCER_TYPE but that is incorrect -->
...
<tr>
<td width="55%"><strong>Cancer-specific Percentile:</strong> `r round(sample_pct_type*100, 1)`</td>
<td width="45%"><strong>Cancer-specific Cohort:</strong> `r Cohort`</td>
</tr>
<tr>
<td width="55%"><strong>Pan-cancer Percentile:</strong> `r round(sample_pct*100, 1)`</td>
<td width="45%"><strong>Pan-cancer cohort:</strong> TCGA Pan-Cancer Atlas 2018 (n=6,446)</td>
</tr>
<tr>
</tr>
</table>
...
ggplot(tcga_tmb_data, aes(tmb)) +
  geom_density(aes(color = "All TCGA"), fill = "grey", alpha = 0.1) + theme_classic() +
  ...
  scale_color_manual(name = "Cohort", breaks = c("All TCGA", "Cohort"), values = c(`All TCGA` = "black", `Cohort` = "#65bc45")) +
  {
    if (sample_tcga %in% external_tmb_data_type$CANCER.TYPE)
      geom_density(data = external_tmb_data_type, aes(color = "Cohort"), fill = "purple", alpha = 0.1)
    else if (sample_tcga %in% tcga_tmb_data_type$CANCER.TYPE)
      geom_density(data = tcga_tmb_data_type, aes(color = "Cohort"), fill = "purple", alpha = 0.1)
  }
