...
defTable <- rbindlist(list(out1, CNV, cbioFusAnno), fill = TRUE) %>%
...
Tx <- rbindlist(list(cbioMAFTx, cbioCNVTx, cbioFusTx), fill = TRUE) %>%
  select(-c(Highest_level, Chromosome, mutation_effect)) %>%
