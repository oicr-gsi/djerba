...
ggplot(tcga_tmb_data, aes(tmb)) +
  ...
  scale_color_manual(name = "Cohort", breaks = c("All TCGA", "External Cohort"), values = c(`All TCGA` = "black", `External Cohort` = "#65bc45")) +
  {
    if (sample_tcga %in% external_tmb_data_type$CANCER.TYPE)
      geom_density(data = external_tmb_data_type, aes(color = "External Cohort"), fill = "purple", alpha = 0.1)
    else
      geom_density(data = tcga_tmb_data_type, aes(color = "External Cohort"), fill = "purple", alpha = 0.1)
  }
