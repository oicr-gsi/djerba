...
class main_base(core_base):
    ...
    def _validate_html_cache_input(self, data):
        if not cc.HTML_CACHE in data:
            ...
            msg = "HTML cache not found in input JSON! "+\
                "Possible version mismatch. Input core version is {0}; ".format(version)+\
                " cache is only generated by version 1.7.0 or higher."
            self.logger.error(msg)
