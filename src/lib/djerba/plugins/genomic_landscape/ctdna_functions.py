...
def run(genomic_landscape_plugin, work_dir, candidate_sites_path=None):
        if candidate_sites_path == None:
            candidate_sites_path = os.path.join(work_dir, constants.MRDETECT_FILTER_ONLY_FILE_NAME)
        ctdna = {}
        ctdna[constants.CTDNA_CANDIDATES] = extract_ctDNA_candidates(genomic_landscape_plugin, work_dir, candidate_sites_path)
        if ctdna[constants.CTDNA_CANDIDATES] >= constants.CTDNA_ELIGIBILITY_CUTOFF:
            ...
            genomic_landscape_plugin.logger.info("Discovered ctDNA candidates: {0}".format(ctdna[constants.CTDNA_CANDIDATES]))
            raise RuntimeError("Unknown number of candidates")
        return(ctdna)

def extract_ctDNA_candidates(genomic_landscape_plugin, work_dir, candidate_sites_path):
