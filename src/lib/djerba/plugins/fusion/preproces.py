...
class prepare_fusions(logger):
    ...
    def add_filter_sortby_read_support(self, df, min_reads):
        ...
        df = filter_and_sort_read_support(df)
    ...
    def add_translocation_notation(self, df):
        ...
        df = correct_translocation_notation(df)
    ...
    def df_for_oncokb_annotator(self, df):
        """
        Makes the dfframe that will be ready for input into the oncokb annotator.
    ...
    def drop_duplicates_merge_columns(self, df):
        ...
        # There may be a None-None fusion. If so, drop it.
        df.drop(df[df['fusion_pairs'] == 'None-None'].index, inplace=True)    
    ...
    def process_mavis(self, mavis_path, tumour_id, min_reads):
        ...
        df_mavis = self.add_read_support(df_mavis)
    ...
    def write_fusion_files(self, SOME_PATH, df_mavis, df_arriba):
        """
        ...
        This function returns df_merged for write_data_fusions_oncokb_txt to use
        """
        ...
        df_merged = self.clean_reading_frames(df_merged)
        ...
        df_merged.to_csv(SOME_PATH, index = False, sep = "\t")
        # Write data_fusions_oncokb.txt to the workspace for annotation task
        df_oncokb.to_csv(SOME_PATH, index = False, sep = "\t")

        return df_merged

    def write_data_fusions_oncokb_txt(self, SOME_PATH, df_merged):
        """
        Writes data_fusions_oncokb.txt to the workspace.
        ...
        """
        # Make a separate table for the OncoKB Annotator 
        df_oncokb = self.df_for_oncokb_annotator(df_merged)
        df_oncokb.to_csv(SOME_PATH, index = False, sep = "\t")
