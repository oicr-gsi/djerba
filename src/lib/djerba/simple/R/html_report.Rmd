...
if (dim(cbioMAF)[1] != 0) {
  cbioMAF <- inner_join(cbioMAF, cytoBand) %>% mutate(OncoKB = ifelse(is.na(Highest_level),
    oncogenic, Highest_level)) %>% inner_join(cytoBand) %>% mutate(HGVSp_Short = base::gsub("p.",
    "", HGVSp_Short))
  ...
  cbioMAF <- cbioMAF %>% mutate(HGVSp_Short = paste0("[", HGVSp_Short, "](", whizbam,
    ")"))
}
...
if (dim(cbioCNV)[1] != 0) {
  cbioCNV <- inner_join(cbioCNV, cytoBand) %>% mutate(OncoKB = ifelse(is.na(Highest_level),
    ...
    Copy_State = base::gsub("^1", "Low-level Amplification", Copy_State), Copy_State = base::gsub("^2",
      "High-level Amplification", Copy_State), Copy_State = base::gsub("^-1",
...
# generate TMB plot here instead of seperately
tmb_file <- paste(cgiTools, "data/tmbcomp.txt", sep = "/")
tmb_data <- read.delim(tmb_file, header = TRUE, stringsAsFactors = F)

# tmb_data_type <- subset(tmb_data, CANCER.TYPE %in% sample_tcga)
tmb_data_type <- tmb_data %>% filter(if (sample_tcga %in% tmb_data$CANCER.TYPE) CANCER.TYPE ==
  sample_tcga else NA)

sample_pct = ecdf(tmb_data$TCGA.tmb)(sampleTMB)
# sample_pct_type = ecdf(tmb_data_type$TCGA.tmb)(sampleTMB)
sample_pct_type <- ifelse(sample_tcga %in% tmb_data$CANCER.TYPE, ecdf(tmb_data_type$TCGA.tmb)(sampleTMB),
  NA)
...
if (dim(cbioMAF)[1] != 0) {
  out1 <- left_join(cbioMAF, CNA) %>% left_join(Expr) %>% mutate(mRNA = (mRNA *
    ...
    tumor_vaf, t_depth, t_alt_count, Copy_State, mRNA, OncoKB) %>% mutate(mRNA = round(mRNA,
    1), tumor_vaf = round(tumor_vaf, 2), tumor_vaf = (tumor_vaf * 100), Copy_State = base::gsub("0",
    "Neutral", Copy_State), Copy_State = base::gsub("^1", "Low-level Amplification",
    Copy_State), Copy_State = base::gsub("^2", "High-level Amplification", Copy_State),
    ...
    rename(Gene = Hugo_Symbol, Type = Variant_Classification, HGVS = HGVSp_Short,
      `Copy State` = Copy_State, `VAF (%)` = tumor_vaf) %>%
  ...
  out1 <- out1[order(ordered(out1$OncoKB, target), -xtfrm(out1$Chromosome)), ]
}
...
defTable <- rbindlist(list(out1, CNV, cbioFusAnno, ExprZ), fill = TRUE) %>% select(Gene,
  Chromosome, OncoKB) %>% left_join(oncoKBsummaries) %>% unique() %>% rename(Summary = summary) %>% mutate(Summary = base::gsub("^NA",
  "No information currently available in OncoKB.", Summary))

defTable <- defTable[order(ordered(defTable$OncoKB, target)),]
...
Tx <- rbindlist(list(cbioMAFTx, cbioCNVTx, cbioFusTx), fill = TRUE) %>% select(-c(Highest_level,
  ...
  "](", "https://www.oncokb.org/gene/", `Gene(s)`, "/", Alteration, ")")) %>% mutate(`Gene(s)` = paste0("[",
  `Gene(s)`, "](", "https://www.oncokb.org/gene/", `Gene(s)`, ")")) %>% mutate(Alteration = base::gsub("/Deep Deletion",
    "/Truncating%20Mutations", Alteration))
...
if (dim(FDA)[1] != 0) {
  FDA %>% knitr::kable(format = "html", escape = FALSE) %>% kable_styling(bootstrap_options = c("striped",
...
if (dim(Other)[1] != 0) {
  Other %>% knitr::kable(format = "html", escape = FALSE) %>% kable_styling(bootstrap_options = c("striped",
    "hover", "condensed")) %>% column_spec(1, italic = T)
}
...
<table width="100%">
  ...
  <tr>
    ...
    <td width="45%"><strong>TCGA Cohort Percentile:</strong> `r round(sample_pct_type, 3)`</td>
  </tr>
</table>
</html>
...
ggplot(tmb_data, aes(TCGA.tmb)) + geom_density(aes(color = "All TCGA"), fill = "grey",
  alpha = 0.1) + theme_classic() + scale_x_continuous(expand = c(0, 0), limit = c(0, max(sampleTMB,25))) +
  scale_y_continuous(expand = c(0, 0)) + geom_vline(xintercept = sampleTMB,
  size = 3) + xlab(" coding mutations per Mb") + ylab("density") + theme(text = element_text(size = 15),
  legend.position = "top") + scale_color_manual(name = "Cohort", breaks = c("All TCGA",
  "Closest TCGA"), values = c(`All TCGA` = "black", `Closest TCGA` = "#65bc45")) +
  {
    if (!is.na(sample_pct_type))
      geom_density(data = tmb_data_type, aes(color = "Closest TCGA"), fill = "purple",
        alpha = 0.1)
  }

ggplot(MAF) + geom_density(aes(x = tumor_vaf), fill = "grey", alpha = 0.5) + geom_rug(aes(x = tumor_vaf,
  ...
  0), limit = c(0, 1)) + scale_y_continuous(expand = c(0, 0))
...
if (!is.null(out1)) {
  out1 %>% remove_rownames() %>% rename(Protein = HGVS) %>% select(-mRNA) %>% knitr::kable(format = "html", escape = FALSE,
    align = "l") %>% kable_styling(bootstrap_options = c("striped", "hover",
    "condensed")) %>% column_spec(1, italic = T)
}
...
**`r CNVcount`** genes were subject to copy number variation, of which **`r CNV_statement`** corresponded to an oncogenic alteration, as defined by OncoKB.


```{r, echo = FALSE, message= FALSE, warning=FALSE}

if (!is.null(CNV)) {
  CNV %>% remove_rownames() %>% knitr::kable(format = "html", escape = FALSE, align = "l") %>%
    kable_styling(bootstrap_options = c("striped", "condensed", full_width = F,
      position = "left")) %>% column_spec(1, italic = T)
}
```
<div id="header1">
...
**`r FUScount`** genes were subject to rearrangement, of which **`r length(unique(cbioFusOnco$Hugo_Symbol))`** corresponded to oncogenic fusions, as defined by OncoKB.

```{r, echo = FALSE, message= FALSE, warning=FALSE}

if (dim(cbioFusAnno)[1] != 0) {
  cbioFusAnno %>% knitr::kable(format = "html", escape = FALSE, align = "l") %>%
...
defTable %>% remove_rownames() %>% knitr::kable(format = "html", escape = FALSE, align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% column_spec(1,
  italic = T)
