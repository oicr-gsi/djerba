...
class main:
    ...
    def run_all(self, input_config, args):
        """Run all Djerba operations in sequence"""
        with tempfile.TemporaryDirectory(prefix='djerba_all_') as tmp:
            ini_path_full = args.config if args.config else os.path.join(tmp, 'djerba_config_full.ini')
            ...
            if args.dir:
                ...
                os.mkdir(report_dir)
            configurer(input_config).run(ini_path_full)
