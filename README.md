...
- The tests use `/.mounts/labs/CGI/software/djerba_test_checkout` to ensure consistent paths for certain data files. Before running tests, ensure the checkouts of `djerba` and `djerba_test_data` in this directory are up to date. (Making the tests more portable is a TODO item.)
